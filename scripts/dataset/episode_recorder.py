...
class Recorder(Node):
    def __init__(self, save_format: str = "array_bgr", jpeg_quality: int = 90,
                 language_instruction: str = "pick up the blue cube"):
        ...
        # Diagnostic: count try_record() calls with missing/out-of-sync data
        self._diag_calls = 0
        ...
        self.image3_sub = self.create_subscription(Image, '/cam_head', self.image3_callback, SENSOR_QOS)
    ...
    def _clear_cache(self):
        ...
        self.latest_image_head = None
        self.latest_state = None
        self.ts_wrist = self.ts_head = self.ts_state = None
        self.episode = []
    ...
    def try_record(self):
        ...
        # ── Diagnostic: report missing / out-of-sync sources every 2 s ──
        ...
        if now - self._diag_last_print >= 2.0:
            missing = [
                src for src, val in [
                    ("cam_wrist (/cam_wrist)", self.latest_image_wrist),
                    ("cam_head  (/cam_head)",  self.latest_image_head),
                    ("robot_state",            self.latest_state),
                ] if val is None
            ]
            if missing:
                self.get_logger().warn(
                    f"[try_record] waiting for: {', '.join(missing)}"
                )
            elif (self.ts_wrist is not None and
                  self.ts_head is not None and self.ts_state is not None):
                span = max(self.ts_wrist, self.ts_head, self.ts_state) \
                     - min(self.ts_wrist, self.ts_head, self.ts_state)
                if span > SYNC_TOL_SEC:
                    self.get_logger().warn(
                        f"[try_record] timestamp spread {span*1000:.0f} ms > "
                        f"{SYNC_TOL_SEC*1000:.0f} ms tolerance – frames out of sync"
                    )
            self._diag_last_print = now
        # ────────────────────────────────────────────────────────────────

        if (self.latest_image_wrist is not None and
            self.latest_image_head is not None and
            self.latest_state is not None and
            self.ts_wrist is not None and
            self.ts_head is not None and self.ts_state is not None):

            tmin = min(self.ts_wrist, self.ts_head, self.ts_state)
            tmax = max(self.ts_wrist, self.ts_head, self.ts_state)
    ...
    def save_episode(self, episode_id: int):
        ...
        try:
            ...
            for i, ts in enumerate(self.episode):
                ...
                valid_imgs = (image_wrist is not None and image_head is not None)
                ...
                # State layout: [0:7]=qpos, [7]=gripper, [8:15]=ee_pose, [15:22]=qvel
                episode_obj["steps"].append({
                    "observation": {
                        ...
                        "qpos":    state_vec[:7],
                        "gripper": state_vec[7:8],
                        "ee_pose": state_vec[8:15],
                        "qvel":    state_vec[15:22],
                    },
                    # action = qpos + gripper + ee_pose of the NEXT timestep (imitation target)
                    "action": (np.asarray(self.episode[i + 1]['state'][:15], dtype=np.float64).ravel()
                               if i + 1 < len(self.episode) else state_vec[:15]),
                    "is_first": i == 0,
...
def main(args=None):
    ...
    parser.add_argument("--instruction", type=str,
                        default=os.getenv("LANGUAGE_INSTRUCTION", "pick up the blue cube"),
                        help="Language instruction label for all episodes in this session")
    ...
    try:
        recorder = Recorder(save_format=parsed.save_format, jpeg_quality=parsed.jpeg_quality,
                            language_instruction=parsed.instruction)
